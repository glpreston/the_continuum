...
class EmotionalMemory:
    ...
    def add_event(
        ...
        log_debug(
            f"[AIRA][emotion_memory] Added event "
            f"(dominant={dominant_emotion}, dims={list(raw_state.keys())})"
        )
    ...
    def _apply_decay_and_recovery(self):
        ...
        log_debug("[AIRA][emotion_memory] Applied emotional decay + recovery")
    ...
    def _update_long_term_emotion(self) -> None:
        ...
        log_debug(
            f"[AIRA][emotion_memory] Long‑term emotion updated: {self.long_term_emotion}"
        )
    ...
    # -----------------------------------------------------------------------
    # EI‑2.0 Memory Influence (NEW)
    # -----------------------------------------------------------------------
    def apply_memory_influence(self) -> Dict[str, float]:
        """
        Computes EI‑2.0 memory influence modifiers based on:
        - long‑term emotional bias
        - volatility
        - confidence
        - recent emotional trend

        Returns a dict used by MetaPersona + style rewrite.
        """

        dominant = self.long_term_emotion or self.short_term_emotion or "neutral"

        # Memory influence strength grows with confidence, shrinks with volatility
        memory_strength = max(0.0, min(1.0, self.confidence * (1.0 - self.volatility)))

        # Basic emotional shaping
        mods = {
            "warmth_boost": 0.0,
            "clarity_boost": 0.0,
            "grounding_boost": 0.0,
        ...
        # Gentle Architect × Warm Guide mapping
        if dominant in ("soft", "calming", "tender"):
            mods["warmth_boost"] += 0.15 * memory_strength
            mods["grounding_boost"] += 0.10 * memory_strength
            mods["pacing_slowdown"] += 0.10 * memory_strength

        elif dominant in ("exploratory", "curious", "bright"):
            mods["clarity_boost"] += 0.10 * memory_strength
            mods["warmth_boost"] += 0.05 * memory_strength

        elif dominant in ("sad", "fatigue", "tired"):
            mods["grounding_boost"] += 0.20 * memory_strength
            mods["pacing_slowdown"] += 0.15 * memory_strength

        # Debug log
        log_debug(
            f"[AIRA][emotion_memory] Memory influence applied: "
            f"dominant={dominant}, strength={memory_strength:.3f}, mods={mods}"
        )
