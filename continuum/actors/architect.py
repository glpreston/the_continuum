...
class Architect(BaseLLMActor):
    ...
    # ---------------------------------------------------------
    # Model selection (legacy — overridden by Senate)
    # ---------------------------------------------------------
    ...
    def propose(
        ...
        # -----------------------------------------------------
        meta = llm_proposal.get("metadata") or {}
        llm_proposal["metadata"] = meta

        meta["selected_model"] = model_to_use
        meta["model_selection_source"] = (
            "ts_selector" if selected_model else "actor_default"
        )
    ...
    def respond(self, prompt: str, **kwargs) -> str:
        """
        Produce a short, single-paragraph structural interpretation.
